...
describe('Text Formatting Preservation', () => {
    ...
    describe('parseHTML with text formatting', () => {
      ...
      `;
            ...
            const boldText = result.resources.texts.find(text => text.content.includes('<strong>bold</strong>'));
            const italicText = result.resources.texts.find(text => text.content.includes('<em>Italic</em>'));
            expect(boldText).toBeDefined();
            expect(italicText).toBeDefined();
        });
    });
});
//# sourceMappingURL=TextFormatting.test.js.map