...
describe('ImageHandlingConfig Component', () => {
    ...
    it('renders with the initial settings', () => {
        ...
        render(_jsx(ImageHandlingConfig, { initialIncludeImages: true, initialOptions: defaultOptions, onChange: onChange }));
    ...
    it('does not show advanced options when images are disabled', () => {
        const onChange = vi.fn();
        render(_jsx(ImageHandlingConfig, { initialIncludeImages: false, initialOptions: defaultOptions, onChange: onChange }));
        // Advanced options button should not be visible
        expect(screen.queryByText('Show Advanced Options')).not.toBeInTheDocument();
    });
});
//# sourceMappingURL=ImageHandlingConfig.test.js.map