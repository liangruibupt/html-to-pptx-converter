...
const mockTableHandler = {
    processTable: vi.fn().mockImplementation((table) => table),
    formatHeaders: vi.fn().mockImplementation((headers) => headers.map(header => ({ text: header, bold: true }))),
    formatRows: vi.fn().mockImplementation((rows) => rows.map(row => row.map(cell => ({ text: cell })))),
    calculateColumnWidths: vi.fn().mockImplementation((table) => Array(table.headers.length).fill(1)),
...
const mockListHandler = {
    ...
    determineBulletType: vi.fn().mockImplementation((list) => list.ordered ? { type: 'number' } : { type: 'bullet', code: '•' }),
...
describe('SlideCreator', () => {
    ...
    describe('createSlides', () => {
        ...
        it('should create slides from sections', async () => {
            ...
            expect(mockPptxGenerator.addSlide).toHaveBeenCalledWith(mockPresentation, 'Test Heading', SlideLayout.WIDE);
        });
        it('should create a default slide if no sections are provided', async () => {
            ...
            expect(mockPptxGenerator.addSlide).toHaveBeenCalledTimes(1);
            expect(mockPptxGenerator.addSlide).toHaveBeenCalledWith(mockPresentation, 'Untitled', SlideLayout.WIDE);
        });
    ...
    describe('createSlideFromSection', () => {
        it('should create a slide with the section title and layout', async () => {
            ...
            expect(mockPptxGenerator.addSlide).toHaveBeenCalledWith(mockPresentation, 'Test Section', SlideLayout.WIDE);
        });
        it('should add elements to the slide', async () => {
            ...
            expect(mockPptxGenerator.addTextElement).toHaveBeenCalledWith(mockSlide, textResource, expect.objectContaining({ x: 0.5, y: 1.5, w: '90%' }));
        });
    ...
    describe('element handling', () => {
        ...
        it('should add image elements to the slide when includeImages is true', async () => {
            ...
            await slideCreator.createSlideFromSection(mockPresentation, section, sampleConfig);
            expect(mockImageHandler.processImage).toHaveBeenCalledWith(imageResource, sampleConfig.imageOptions);
            expect(mockPptxGenerator.addImageElement).toHaveBeenCalledWith(mockSlide, imageResource, expect.objectContaining({ x: 1, y: 2 }));
        });
        ...
        it('should add table elements to the slide', async () => {
            ...
            expect(mockPptxGenerator.addTableElement).toHaveBeenCalledWith(mockSlide, expect.objectContaining({
                headers: ['Header 1', 'Header 2'],
                rows: [
                    ['Row 1, Cell 1', 'Row 1, Cell 2'],
                    ['Row 2, Cell 1', 'Row 2, Cell 2']
        ...
        it('should add list elements to the slide', async () => {
            ...
            expect(mockPptxGenerator.addListElement).toHaveBeenCalledWith(mockSlide, expect.objectContaining({
        ...
        it('should add link elements to the slide when preserveLinks is true', async () => {
            ...
            expect(mockPptxGenerator.addLinkElement).toHaveBeenCalledWith(mockSlide, processedLink, expect.objectContaining({
                x: 0.5,
                y: 2,
                w: '90%',
        ...
        it('should apply different layout positions based on slide layout', async () => {
            ...
            expect(mockPptxGenerator.addTextElement).toHaveBeenCalledWith(mockSlide, textResource, expect.objectContaining({ x: 0.5, y: 1.5, w: '95%' }));
        });
        it('should apply image processing options when available', async () => {
            ...
            expect(mockImageHandler.processImage).toHaveBeenCalledWith(imageResource, configWithImageOptions.imageOptions);
            expect(mockPptxGenerator.addImageElement).toHaveBeenCalledWith(mockSlide, processedImage, expect.objectContaining({
                x: 1,
                y: 2,
                w: 3, // 300 / 100
                h: 1.8 // 180 / 100
...
//# sourceMappingURL=SlideCreator.test.js.map